<x-layouts.app :title="__('Edit Produk')">
    ...
    <form method="POST" action="{{ route('product1.update', $product1->id) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ old('name', $product1->name) }}"
        ...
        <div class="mb-4">
            ...
            <input type="text" name="sku" value="{{ old('sku', $product1->sku) }}"
                class="border rounded w-full p-2">
            @error('sku')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-4">
            ...
            <input type="number" name="price" value="{{ old('price', $product1->price) }}" step="0.01"
                class="border rounded w-full p-2">
            @error('price')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-4">
            ...
            <input type="number" name="stock" value="{{ old('stock', $product1->stock) }}"
                class="border rounded w-full p-2">
            @error('stock')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-4">
            ...
            <input type="url" name="image_url" value="{{ old('image_url', $product1->image_url) }}"
                class="border rounded w-full p-2">
            @error('image_url')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
            ...
            <select name="brand_id" class="border rounded w-full p-2 bg-white font-medium text-gray-800" required>
                ...
                @foreach ($brands as $brand)
                    <option value="{{ $brand->id }}"
                        {{ old('brand_id', $product1->brand_id) == $brand->id ? 'selected' : '' }}>
                        {{ $brand->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="os_type_id" class="border rounded w-full p-2 bg-white font-medium text-gray-800" required>
                ...
                @foreach ($osTypes as $os)
                    <option value="{{ $os->id }}"
                        {{ old('os_type_id', $product1->os_type_id) == $os->id ? 'selected' : '' }}>
                        {{ $os->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <label class="inline-flex items-center">
                <input type="checkbox" name="is_active" value="1" class="mr-2"
                    {{ old('is_active', $product1->is_active ?? false) ? 'checked' : '' }}>
